...
class Forest {
 ...
 public:
  ...
  Rcpp::List get_trees(Rcpp::IntegerVector tree_idx);
  void set_trees(Rcpp::List tree_lists, Rcpp::IntegerVector tree_idx);
...
//
// Near the other free-function declarations in the same header (directly
// below `arma::vec predict(Node* node, const arma::mat& X, const Hypers&
// hypers);`), add these two lines:
